...
class Polyfordriverfunction{
  ...
  static getPrice(index){
    ...
    if(index==0) return double.parse(GetUtils.numericOnly(distance!)) * nightFactor;
    if(index==1) return double.parse("${GetUtils.numericOnly(distance!)}")* 1.5 *nightFactor;
  ...
  static makePolyLine(latOrigin,lngOrigin,latDestination,lngDestination) async {
    ...
    markers.add(Marker(markerId: MarkerId('loc'),position: LatLng(double.parse("$latOrigin"),double.parse("$lngOrigin")),infoWindow: InfoWindow(title: "My Loc",),icon: PolyFunction.cars!));//icon: cars!
    ...
    polyLines.add(Polyline(
        ...
        geodesic: true,jointType: JointType.round,patterns: [PatternItem.dash(8), PatternItem.gap(5),]
    ),);
